...
Examples were extracted from the DES-Mutation KB, minor differences might exist
...
terms/concepts (in the knowledgebase called the “Associated terms” or
“Term Pairs”) are inferred based on their co-occurrence. For example,
...
DES_Mutation is a web based application. To start exploring, inside
your browser open one of the URLs on the documentation [home
page](index.md). This will take you to the knowledgebase opening
screen as shown in Figure 4 for *DES-mutation*. To navigate through
the knowledgebase, you can use the main menu as shown in the figure as
the top bar.
...
- Explore Hypotheses: automated method for finding evidence in the literature to
support hypothetical relationships.
- KOBAS Pathways: shows KOBAS (KEGG Ontology-Based Annotation System) pathways relevant to the
genes/proteins mined within the knowledgebase text.
- Show Literature: enable literature exploration by using terms and dictionaries as
keywords.
- About: shows background information about the knowledgebase.

![Navigation menu](images/des_nav.png "Figure 5. Main navigation menu")

## Enriched Terms exploration mode
...
term within this knowledgebase. For example: Glykane Glycane, Glykan
polisacarido etc. By clicking on the frequency number a new window
...
* Term Link sources: a concept association sources are displayed as a pie chart.
...
![Terms co-occurrence table](images/des_tco.png "Figure 7. Terms co-occurrence table")
...
is much richer and displayed as a table as shown in Figure 8.

![Enriched term pairs display](images/des_etp.png "Figure 8 Enriched term pairs display")
...
of term A name variations occurrence within the knowledgebase.  • B
Freq: term B frequency. A total count of term B name variations
...
The table can be ordered by any of its columns, and cut-offs can be applied to any of its numerical
columns at the table header or footer. Terms are also searchable from
the Filter input in the header. Also, individual dictionaries can be
searchable at the table header/footer text inputs.  Associations are
first captured through raw co-occurrence counts, and then two more
accurate measures are used to rank the strength of these
associations. The Point-wise Mutual Information measure is a
well-known metric for quantifying association in information theory
and statistics:

![Point-wise Mutual Information measure](images/des_pmi.png "Figure 8. Point-wise Mutual Information measure")
...
“Mutations (tmVar)” and “DOID Ontology (Bioportal)”. In addition,
mutations are filtered by the SNP mutation 'rs16969968'. The result is
list of associations between this SNP and diseases.

![Associations mining example](images/des_assocs.png "Figure 9 Associations mining example")
...
option from a context menu. Figure 10 shows the actual network. On the
...
![Visualizing connections between terms](images/des_conns.png "Figure 10 Visualizing connections between terms")

The main visualization menu (at the top) consists of number of
dropdown as shown in Figure 11.

![Visualization menus](images/des_vismenu.png "Figure 11 Visualization menus")

Links filter: allows filtering links between terms according to the
following criteria:

- Minimum PubMed Links: show only links where term
co-occurrence exceeds the specified threshold.
- Maximum Sentence
Distance: show only associations where two terms co-occur within, at
most, the specified number of sentences from each other, at least once
within the edge.
- Maximum Nodes: when specified, only the top
'Maximum Nodes' associated with the terms are displayed. The default
is 50.
- Search Nodes: allows search for a particular term by its
representative term as shown in the network, e.g. for genes/proteins
this would be an official symbol or an official name.
- Include/Exclude Imported Associations: the default is to show only
text-mined associations. By enabling this check box, the system will
also show pre-defined associations imported from external sources,
such as: Bio grid Gene-Gene links, Reactome Gene-Pathway links, etc.
- Imported Maximum Nodes: similar to Maximum Nodes but applied to
imported links.
...
Figure 11 Graph layouts: circle, force-directed non-proportional,
force-directed proportional, radial, tree

Export the network:
exporting the network graph into a portable image format (.png).

Reset the network: return to the original node and its associations.

### Literature exploration mode

This page allows literature search by
selected terms or dictionaries. Figure 12 shows such search where
...
![Literature search](images/des_lits.png "Figure 12 Literature search")
